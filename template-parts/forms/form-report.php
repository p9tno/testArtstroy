<?php
$data_download = '';
if (get_field('data_download_report', 'option')) { 
    $data_download = ' data-download="'.get_field('data_download_report', 'option').'"' ;
} 
?>
...
<div class="report__form">
    ...
    <form class="form form_sm" action="<?php echo get_template_directory_uri() . '/send.php'; ?>" <?php echo $data_download;   unset($data_download); ?> >
        ...
        <input type="hidden" name="subject" value="Скачайте пример реальной сметы на ремонт с полными графиками работ" />
        ...
        <div class="form__row">
            <button class="btn btn_didisabled_js" type="submit"><span>Скачать смету</span></button>
        </div>
